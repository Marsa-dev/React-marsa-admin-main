...
import { termswithlanguage } from '../../utils/termsconditions';

export default function Terms() {
  const [languages, setLanguages] = useState('English');

  const terms = termswithlanguage(languages);
  ...
  return (
    <>
      <Helmet>
        <title> Terms and Conditions | Marsa </title>
      </Helmet>
      ...
      <Container maxWidth="xl">
        ...
        <Card sx={{ padding: '40px', direction: languages === 'Arabic' ? 'rtl' : 'ltr' }}>
          <Stack spacing={2}>
            ...
            <Stack p={2} spacing={1}>
              <Typography fontSize={'26px'} fontWeight={600} textAlign={'start'}>
                {terms.h2}
              </Typography>
              <Typography fontSize={'14px'} fontWeight={400} textAlign={'start'}>
                {terms.p2}
              </Typography>
            </Stack>
